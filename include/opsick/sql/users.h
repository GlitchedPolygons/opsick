...
static const char opsick_sql_create_user[] = "INSERT INTO users (pw, exp_utc, body, body_sha512, public_key_ed25519, encrypted_private_key_ed25519, public_key_curve448, encrypted_private_key_curve448) VALUES ($1::text, $2::bigint, $3::text, $4::text, $5::text, $6::text, $7::text, $8::text)";
static const char opsick_sql_delete_user[] = "DELETE FROM users WHERE id = $1::bigint";

static const char opsick_sql_set_user_pw[] = "UPDATE users SET pw = $1::text, lastmod_utc = (SELECT UTC_NOW()) WHERE id = $1::bigint";
